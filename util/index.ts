import {
	...
	Rules,
	validate,
} from "@/base/components/form/validate"
...
>(
	...
	const ret = ref<Return | undefined>(
		initValue,
	) as Ref<Return | undefined>
	watch<T, boolean>(
		watcher,
		async (val, old) => {
			if (val !== old) {
...
export function useWatch(
	...
	const watch =
		_watch instanceof Array ? _watch : [_watch]
...
export function useWatchValidate<
	D extends object,
>(
	entity: MaybeRef<D>,
	rules: Rules<D>,
	...
	Object.keys(rules).forEach((k) => {
		watch(
			...
			async () => {
				const rule = rules[k as keyof D] as Rule<
					D,
					D[keyof D]
				>
				if (rule) {
