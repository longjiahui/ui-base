...
export function min(
  ...
  return {
    validator(value) {
      if (value) {
        if (value instanceof Array) {
          return value.length >= min ? "" : message || `不能少于${min}个元素`
...
export function max(
  ...
  return {
    ...
    validator(value) {
      if (value) {
        if (value instanceof Array) {
          return value.length <= max ? "" : message || `不能多于${max}个元素`
        } else if (typeof value === "string" || typeof value === "number") {
          if (type === "string") {
...
export function alphabetOrNumber(): Rule {
  return {
    validator(value) {
      if (value) {
        return /^[A-Za-z0-9]+$/.test(value) ? "" : "请使用英文和数字组合"
      }
    },
  }
}
  ...
  return {
    validator(value) {
      if (value) {
        ...
        return /^(0|(-?0\.\d*[1-9])|(-?([1-9]\d*(\.\d+)?)))$/.test(value)
          ? ""
          : "请输入数值"
      }
    },
...
export const area = () => $and(fixed1(), positiveNumber())
...
export const money = () => $and(_fixed2())
...
export function password(): Rule {
  return $and(required(), alphabetOrNumber(), range(8, 14))
}
